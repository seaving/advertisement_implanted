...
int CalMd5(char *src, int src_len, char *md5str)
{
 	...
 	memcpy(psrc, TOKEN_KEY, TOKEN_KEY_LEN);
	myMd5((unsigned char *)src, psrc - src + TOKEN_KEY_LEN, (unsigned char *)md5sum);
...
{
	return myMd5File(path, offset, dataLen, TOKEN_KEY, TOKEN_KEY_LEN, md5);
}

int qcMd5(char *md5)
{
	char tmp[100];
	sprintf(tmp, "%s%s", GET_DEV_NO, GET_WAN_MAC);
	return CalMd5(tmp, strlen(tmp), md5);
}
