...
void struct_http_x_auth_header(char * method, 
		char * path, char * host, int port, 
		char *data, char *content_type, 
		char * header_buff, int header_buff_size)
{	
	char token[33] = {0};
	char *mac = GET_WAN_MAC;

	CalMd52(data, strlen(data), "qianchen", path, X_AUTH_TOKEN_KEY, X_AUTH_TOKEN_KEY_LEN, token);
	if (strcmp("GET", method) == 0)
	{
		snprintf(
			header_buff, header_buff_size, 
			"GET %s HTTP/1.1\r\n"
			"Host: %s:%d\r\n"
			"x-auth: %s;%s;%s;%s\r\n"
			"\r\n",
...
int web_html_jump(int client_sock, char * to_url)
{
    ...
    if (socket_send(client_sock, header_buff, len, 15) != len)
    {
        return -1;
    }
    return 0;
...
int get_user_agent(char *header, int header_len, char *buf, int bufsize, int *port)
{
	char *p = strnaddr_case(header, header_len, "\r\nUser-Agent:", 15);
	if (! p)
		return -1;
...
{
	char *p = strnaddr_case(header, headerlen, "\r\nContent-Length:", 17);
...
int isHtml(char *header_buff, int header_len, char *uri, int urilen)
{
	...
	if (find_str_case(header_buff, header_len, "Accept-Encoding: gzip", 21) >= 0)
	{
		return 0;
	}
	if (find_str_case(header_buff, header_len, "Content-Encoding: gzip", 21) >= 0)
	{
		return 0;
	}

	char *pp = NULL;
	if (((pp = strnaddr_case(uri, urilen, ".js", 3)) && (*(pp+3) == '?' || *(pp+3) == '\0'))
	|| ((pp = strnaddr_case(uri, urilen, ".css", 4)) && (*(pp+4) == '?' || *(pp+4) == '\0'))
	|| ((pp = strnaddr_case(uri, urilen, ".json", 5)) && (*(pp+5) == '?' || *(pp+5) == '\0')))
	{
		return 0;
	}
	...
	if (ret >= 0)
	{
		...
		if (pT)
		{
			...
			if (0 < size)
			{
				...
				tmp[size] = '\0';
				if (! straddr_case(tmp, "text") && ! straddr_case(tmp, "html"))
				{
					html_flag = 0;
				}
				else
	...
	{
		if (isStrnexit_case(uri, urilen, ".htm", 4) //html htm
		|| isStrnexit_case(uri, urilen, ".jhtm", 5)
		|| isStrnexit_case(uri, urilen, ".shtm", 5)
		|| isStrnexit_case(uri, urilen, ".jsp", 4))
			html_flag = 1;
		...
		if (! html_flag)
		{
			...
			if (j > 0)
			{
				if (p)
				{
					...
					tmp[p - (uri + j)] = 0;
					if (isStrnexit_case(tmp, p - (uri + j), "htm", 3)
					|| isStrnexit_case(tmp, p - (uri + j), "jsp", 3))
					{
						html_flag = 1;
					}
				}
				...
				{
					if (isStrnexit_case(uri+j, urilen - j, "htm", 3)
					|| isStrnexit_case(uri+j, urilen - j, "jsp", 3))
					{
						html_flag = 1;
					}
				}
