...
int CalMd5(char *src, int src_len, char *md5str)
{
 	...
 	memcpy(psrc, DATA_TOKEN_KEY, DATA_TOKEN_KEY_LEN);
	myMd5((unsigned char *)src, psrc - src + DATA_TOKEN_KEY_LEN, (unsigned char *)md5sum);
...
{
	return myMd5File(path, offset, dataLen, DATA_TOKEN_KEY, DATA_TOKEN_KEY_LEN, md5);
}
