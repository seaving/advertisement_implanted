...
int connect_noblock(int clientfd, struct sockaddr_in *serveraddr)
{
	...
	while (1)
	{
		...
		{
#ifdef DEBUG_SOCKET
			LOG_NORMAL_INFO("(s:%d) connect time out.\r\n", clientfd);
#endif
			return -1;
		}
#ifdef DEBUG_SOCKET
		 ...
		 && (FD_ISSET(clientfd, &socketWr) > 0))
		{
			...
			{
#ifdef DEBUG_SOCKET	
				LOG_NORMAL_INFO("Get SO_ERROR error, connect failt.\r\n");
#endif
				return -1;
			}
#ifdef DEBUG_SOCKET
			LOG_NORMAL_INFO("Get ERROR parma:%d, %d\r\n", iError, ret);
#endif

			if (iError != 0)
			{
#ifdef DEBUG_SOCKET
				LOG_NORMAL_INFO("ERROR != 0; connect failt.\r\n");
#endif
		...
		{
#ifdef DEBUG_SOCKET
			LOG_NORMAL_INFO("socket:%d read & nowrite, connect failt.\r\n", clientfd);
#endif
			return -1;
		}
	}
...
int connect_host_noblock(char *hostname, int port)
{
    ...
    if ((rc = open_clientfd_noblock(NULL, hostname, port)) < 0)
    {
		if (rc == -1)
		{			
			LOG_NORMAL_INFO("%s:%d socket create error!!!\n", hostname, port);
		    LOG_PERROR_INFO("Open_clientfd Unix error");
		}
		else if (rc == -2)
		{
		...
		{
			LOG_NORMAL_INFO("%s:%d connect error!!!\n", hostname, port);		
		    LOG_PERROR_INFO("Open_clientfd Unix error");
		}
		else if (rc == -5)
		{
			LOG_NORMAL_INFO("filter host : %s.\n", hostname);
		}
    }
    return rc;
}
