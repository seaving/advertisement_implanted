...
char *libpcap_lookupdev(char *dev, char *errbuf)
{
	...
	if (cap_dev == NULL)
	{
		...
		if (cap_dev == NULL)
		{
			VAL_LOG("Couldn't find default device %s: %s\n", dev, errbuf);
			return NULL;
		}
	}
...
int libpcap_lookupnet(char *dev, bpf_u_int32 *net, bpf_u_int32 *mask, char *errbuf)
{
	...
	{
		VAL_LOG("Couldn't get netmask for device %s: %s\n", dev, errbuf);
        *net = 0;
        *mask = 0;
        return -1;
    }
...
pcap_t *libpcap_open_live(char *dev, int snaplen, int promisc, int to_ms, char *errbuf)
{
    ...
    if (handle == NULL)
	{
        VAL_LOG("Couldn't open device %s: %s\n", dev, errbuf);
        return NULL;
    }
...
int libpcap_filter_set(pcap_t *handle, char *filter_exp, struct bpf_program *filter, bpf_u_int32 net)
{
	...
	{
        VAL_LOG("Couldn't parse filter %s: %s\n", filter_exp, pcap_geterr(handle));
        return -1;
    }

    if (pcap_setfilter(handle, filter) == -1)
	{
        VAL_LOG("Couldn't install filter %s: %s\n", filter_exp, pcap_geterr(handle));
        return -1;
    }
...
void libpcap_loop(pcap_t *handle, int num_packets, pcap_handler callback, char *arg)
{
