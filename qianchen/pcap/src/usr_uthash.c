...
void *uthash_find(hashtable_t **hashHead, char *ht_key)
{
	...
	if (! out)
	{
		//VAL_LOG("the key(%s) is not exist in hash!\n", ht_key);
		return NULL;
	}
...
void uthash_destroy(hashtable_t **hashHead)
{
	...
	{
		VAL_LOG("[safe traversal]key:%s, value:%s\n", current->key, (char *)current->value);
...
int test_uthash()
{
	...
	for (i = 0; i < 11; i ++)
	{
		...
		VAL_LOG("find key:%s, value:%s\n", out->key, (char *)out->value);
	}

	//不安全遍历
	for (current = hashHead; current != NULL; current = (hashtable_t *)(current->hh.next))
	{
		VAL_LOG("[unsafe traversal]key:%s, value:%s\n", current->key, (char *)current->value);
	}
