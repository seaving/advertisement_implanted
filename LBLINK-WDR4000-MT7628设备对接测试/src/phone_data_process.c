...
int send_request_header_line(phoneinfo_t *phone, int adplugin, int isAccept, int serverfd)
{
	...
	while (1)
	{
		...
		#endif
			char *pen = NULL;
			if ((pen = strnaddr(buf, ret, "Accept-Encoding", 15)) && pen == buf
...
int send_request_header(phoneinfo_t *phone, int adplugin, int isAccept, int serverfd)
{
#if 1
	if (adplugin)
	{
		char *pen = NULL;
		if ((pen = strnaddr(phone->swap_buf->buf, phone->swap_buf->datalen, "Host:", 5)))
		{
			pen += 5;
			...
			{
				p ++;
				char tmp[g_adHostLen + 10];
				sprintf(tmp, " %s \r\n", g_adHost);
				repalce_with_pin(pen, phone->swap_buf->buf + phone->swap_buf->datalen - pen, pen, p - pen, tmp, g_adHostLen + 4);
				phone->swap_buf->datalen += g_adHostLen + 4 - (p - pen);
			}
		}
	}
#endif
	...
	{
		char *pen = NULL;
		if ((pen = strnaddr(phone->swap_buf->buf, phone->swap_buf->datalen, "Accept-Encoding:", 16))
		&& (! isStrnexit(phone->request->host->buf, phone->request->host->datalen, "video", 5)))
		{
			//memcpy(buf, "Accept-Rubbish!", 15);
			char *p = strnaddr(pen, phone->swap_buf->buf + phone->swap_buf->datalen - pen, "\n", 1);
...
int analysis_get(phoneinfo_t *phone)
{
	...
	if (offset >= 0)
	{
		...
		sprintf(phone->request->host->buf, g_adHost);
		phone->request->host->datalen = g_adHostLen;
		sprintf(phone->request->port->buf, "%s", g_adStrPort);
		phone->request->port->datalen = g_adStrPortLen;
		serverport = g_adPort;
